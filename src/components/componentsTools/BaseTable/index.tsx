...
const BaseTable = (props: IBaseTableProps, ref: any) => {
  ...
  const fetchApiMemoized = useCallback(
    (extraParams?: any, refresh?: boolean) => {
      ...
      function compareApiParms() {
        let _apiParams = apiParams();
        if (paginationData?.pageSize) {
          if (
            paginationData?.pageSize !== _apiParams?.pageSize ||
            paginationData?.pageNumber !== _apiParams?.pageNumber
          ) {
            return false;
          }
        }
      ...
      // 异步分页时调用
      if (asyncPagination) {
        (fetchPageApi ? fetchPageApi(finalParams) : fetchApi(finalParams)).then((res: any) =>
  ...
  if (paginationData) {
    ...
    pagination = paginationCb ? paginationCb(paginationData, defaultPageConfig) : defaultPageConfig;
  }
